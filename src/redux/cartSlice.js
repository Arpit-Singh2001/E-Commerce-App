...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    // create an action to removeCartItem from cart
    resetCart: (state, action) => {
      //   state.cart = [];
      const curKey = action.payload.key;
      state.cart = state.cart.filter((item) => item.key !== curKey);
    },
    // create an action to removeCartItem when payment sucess
...
export const { addToCart, removeFromCart, resetCart, resetAllCart } =
  cartSlice.actions;
